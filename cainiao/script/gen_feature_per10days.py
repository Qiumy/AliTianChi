...
def genFeature(itemID,dateID,monthdata,afterData):
	...
	sum10_collect_uv = sum(monthdata["collect_uv"][5:])

	sum5_cart_ipv = sum(monthdata["cart_ipv"][:5])
	sum10_cart_ipv = sum(monthdata["cart_ipv"][5:])

	sum5_pv_uv = sum(monthdata["pv_uv"][:5])
	sum10_pv_uv = sum(monthdata["pv_uv"][5:])
...
def writeData(itemFeatureList):
	with open("../data2/all/item_feature_per10.csv", "a") as f:
		write = csv.writer(f)
