...
date_id = ["20141101","20141116","20141201","20141216",\
		   "20150101","20150116","20150301","20150316",\
		   "20150401","20150416","20150501","20150516",\
		   "20150601","20150616","20150701","20150716",\
		   "20150801","20150816","20150901","20150916",\
		   "20151001","20151016","20151101","20151116"]
def calFeature():
	direction = "../data4/all/item"
	file_list = os.listdir(direction)
	for file_name in file_list:
		cinData(file_name)

def cinData(itemID):
	item = pd.read_csv("../data4/all/item/"+itemID)
	itemID = itemID[:-4]
	item["date"] = [datetime.datetime.strptime(str(x), "%Y%m%d") for x in item['date']]
	itemsort = item.sort_values(by="date")
	for date in date_id:
		begindate = datetime.datetime.strptime(date, "%Y%m%d")
		enddate = datetime.datetime.strptime(date, "%Y%m%d") + datetime.timedelta(days=14)
...
def genFeature(itemID,dateID,monthdata,afterData):
	...
	sum15_collect_uv = sum(monthdata["collect_uv"][10:])

	sum5_cart_ipv = sum(monthdata["cart_ipv"][:5])
	sum10_cart_ipv = sum(monthdata["cart_ipv"][5:10])
	sum15_cart_ipv = sum(monthdata["cart_ipv"][10:])

	sum5_pv_uv = sum(monthdata["pv_uv"][:5])
	sum10_pv_uv = sum(monthdata["pv_uv"][5:10])
	sum15_pv_uv = sum(monthdata["pv_uv"][10:])

	sum5_qty_gmv = sum(monthdata["qty_gmv"][:5])
	sum10_qty_gmv = sum(monthdata["qty_gmv"][5:10])
	sum15_qty_gmv = sum(monthdata["qty_gmv"][10:])

	sum5_qty_alipay = sum(monthdata["qty_alipay"][:5])
	sum10_qty_alipay = sum(monthdata["qty_alipay"][5:10])
	sum15_qty_alipay = sum(monthdata["qty_alipay"][10:])

	trendFeature = [sum10_pv_uv-sum5_pv_uv,sum15_pv_uv-sum10_pv_uv,\
					sum10_cart_ipv-sum5_cart_ipv,sum15_cart_ipv-sum10_cart_ipv,\
					sum10_collect_uv-sum5_collect_uv,sum15_collect_uv-sum10_collect_uv,\
					sum10_qty_gmv-sum5_qty_gmv,sum15_qty_gmv-sum10_qty_gmv,\
					sum10_qty_alipay-sum5_qty_alipay,sum15_qty_alipay-sum10_qty_alipay
					]
...
def writeData(itemFeatureList):
	with open("../data4/all/item_feature_all0509_3.csv", "a") as f:
		write = csv.writer(f)
